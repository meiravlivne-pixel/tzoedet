...
function clearForm(){minutes.value='';state={};document.querySelectorAll('#effort .choice,#pain .choice').forEach(c=>c.classList.remove('selected'));}
startBtn.onclick=()=>{editIndex=null;clearForm();walkForm.classList.remove('hidden');};
saveBtn.onclick=()=>{const w={date:new Date().toLocaleDateString('he-IL'),minutes:+minutes.value,e:state.e,p:state.p};if(editIndex===null)walks.push(w);else walks[editIndex]=w;localStorage.setItem('walks',JSON.stringify(walks));walkForm.classList.add('hidden');clearForm();render();};
...
window.editWalk=i=>{editIndex=i;const w=walks[i];minutes.value=w.minutes;state.e=w.e;state.p=w.p;document.querySelectorAll('#effort .choice').forEach((c,idx)=>c.classList.toggle('selected',idx===w.e));document.querySelectorAll('#pain .choice').forEach((c,idx)=>c.classList.toggle('selected',idx===w.p));walkForm.classList.remove('hidden');}
